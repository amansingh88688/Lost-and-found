...
<head>
    ...
    <title>Verify OTP | Lost and Found Portal</title>
...
<body oncontextmenu="return false;" onload="noBack();" onpageshow="if (event.persisted) noBack();" onunload="">
    ...
    <div class="container">
        <div class="form form--signup">
            ...
            <div class="form--heading heading2">Enter the OTP sent to your smail, to set new password.</div>
            <form method="post">
...
if ($_SERVER['REQUEST_METHOD'] ==  'POST') {
    ...
    if ($result) {
        ...
        if ($number == 1) {
            ...
            $otp = $row2['otp'];
            if ($inputOTP == $otp) {
                $sql = "UPDATE `users` SET `otp_expired` = 'yes' WHERE `roll` = '$roll';";
                $result = mysqli_query($con, $sql);
                header('refresh: 0.01; url=reset_pass.php');
            } else {
